...
const VENDOR_DEVICE_LIST = [
  ...
  { vid: 0x0b05, pid: 0x190e, resetCompleteFallback: true }, // ASUS USB-BT500 (Realtek RTL8761B)
...
function BluetoothHciSocket () {
  this._isUp = false;
  this._resetCompleteFallback = false;
...
BluetoothHciSocket.prototype.bindUser = function (devId, params) {
  ...
  this._resetCompleteFallback = this._needsResetCompleteFallback(this._usbDevice);
...
BluetoothHciSocket.prototype._needsResetCompleteFallback = function (usbDevice) {
  const deviceDescriptor = usbDevice && usbDevice.deviceDescriptor;

  if (!deviceDescriptor) {
    return false;
  }

  return VENDOR_DEVICE_LIST.some(device => {
    return device.resetCompleteFallback === true &&
      deviceDescriptor.idVendor === device.vid &&
      deviceDescriptor.idProduct === device.pid;
  });
};
...
BluetoothHciSocket.prototype.onHciEventEndpointData = function (data) {
  ...
  if (pktLen <= (this._hciEventEndpointBuffer.length - 2)) {
    ...
    const isStandardResetComplete = buf.length === 6 &&
      (buf.toString('hex') === '0e0401030c00' || buf.toString('hex') === '0e0402030c00');
    const isFallbackResetComplete = !this._isUp &&
      this._resetCompleteFallback &&
      buf.length >= 6 &&
      buf.readUInt8(0) === 0x0e &&
      buf.readUInt8(5) === 0x00;

    if (this._mode === 'raw' && (isStandardResetComplete || isFallbackResetComplete)) {
      if (isFallbackResetComplete && !isStandardResetComplete) {
        debug('reset complete (ASUS USB-BT500 fallback)');
      } else {
